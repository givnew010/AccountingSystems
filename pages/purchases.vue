...
const products = ref([
  { id: 1, name: 'منتج 1', costPrice: 8.00 },
  { id: 2, name: 'منتج 2', costPrice: 20.00 },
  { id: 3, name: 'منتج 3', costPrice: 12.00 }
])

const invoices = ref([
  {
    id: 1, no: 1, number: 'PUR-001', date: '2024-01-14', supplierId: 1, supplier: { name: 'شركة الأمانة للتوريدات' },
    paymentMethod: 'credit', supplierInvoiceNo: 'S-456', notes: '',
    items: [{ productId: 1, unit: 'piece', quantity: 10, price: 8.00, discount: 0, total: 80.00 }],
    total: 80.00, status: 'posted'
  }
])
...
const saveInvoice = () => {
  const supplier = suppliers.value.find(s => s.id == currentData.value.supplierId)
  ...
  if (selectedItemId.value) {
    ...
    const newInv = { ...currentData.value, id: newId, no: newId, supplier, total }
