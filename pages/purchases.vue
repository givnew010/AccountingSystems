<template>
  <div class="min-h-screen bg-gray-100 overflow-hidden">
    <div class="flex flex-row h-screen">
      <ListSection title="فواتير المشتريات" :items="invoicesForList" search-placeholder="البحث في الفواتير..."
        ...
        <form @submit.prevent="saveInvoice" class="space-y-4 pb-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="currentData.supplierInvoiceNo" type="text" :disabled="isViewMode"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100" />
            </div>
          </div>
...
const getStatusLabel = (status) => purchasesStore.getStatusLabel(status)
const selectItem = (id) => purchasesStore.selectItem(id)
const newInvoice = () => purchasesStore.newInvoice()
const editInvoice = () => purchasesStore.editInvoice()
const cancelEdit = () => purchasesStore.cancelEdit()
const addItem = () => purchasesStore.addItem()
const removeItem = (index) => purchasesStore.removeItem(index)
const updateProduct = (index) => purchasesStore.updateProduct(index)
const calcTotal = (index) => purchasesStore.calcTotal(index)

const saveInvoice = () => {
  const res = purchasesStore.saveInvoice()
...
const deleteInvoice = () => {
  ...
  showMessage({
    title: 'تأكيد الحذف',
    message: `هل أنت متأكد من حذف فاتورة المشتريات «${currentData.value.number}»؟`,
    cancelText: 'إلغاء',
    confirmText: 'حذف',
    onCancel: () => {},
    onConfirm: () => {
      const ok = purchasesStore.confirmDeleteSelected()
...
const postInvoice = () => {
  const ok = purchasesStore.postInvoice()
