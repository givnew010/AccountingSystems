    ...
    <div class="p-6">
      ...
      <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <button v-for="option in importOptions" :key="option.type" @click="selectImportType(option)"
            :class="['p-4 border-2 rounded-lg text-center transition-colors', selectedType === option.type ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300 text-gray-700']">
      ...
      <div v-if="selectedType" class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
          ...
          <div class="mb-4">
            ...
            <input id="file-upload" type="file" class="hidden" accept=".csv,.xlsx,.xls" @change="handleFileSelect" />
          </div>
          <p class="text-sm text-gray-500">دعم ملفات CSV و Excel (xlsx, xls) - الحد الأقصى 10MB</p>
          <p v-if="selectedFile" class="mt-2 text-sm text-green-600">تم اختيار الملف: {{ selectedFile.name }}</p>
        </div>
      ...
      <div v-if="showMapping" class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="mb-4">
          <p class="text-sm text-gray-600 mb-4">ربط أعمدة الملف بالحقول في النظام. الأعمدة المطلوبة: {{ requiredFields.join(', ') }}</p>
        </div>
        ...
        <div class="mt-6 flex gap-4">
          <button @click="importData" :disabled="importing || !isMappingValid"
          ...
          <button @click="cancelImport"
            class="px-6 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">إلغاء</button>
        </div>
      </div>
      ...
      <div v-if="importResults" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="flex gap-4">
          <button v-if="importResults.errors > 0" @click="downloadErrors"
            class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">تحميل تقرير الأخطاء</button>
          <button @click="resetImport"
            class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">استيراد جديد</button>
        </div>
      </div>
    </div>
  </div>
...
import { UsersIcon, CubeIcon, TagIcon, DocumentArrowUpIcon } from '@heroicons/vue/24/outline'

const importOptions = ref([
  {
    type: 'customers', title: 'العملاء', description: 'استيراد بيانات العملاء', icon: UsersIcon,
  ...
  {
    type: 'products', title: 'المنتجات', description: 'استيراد بيانات المنتجات', icon: CubeIcon,
  ...
  {
    type: 'categories', title: 'الفئات', description: 'استيراد بيانات الفئات', icon: TagIcon,
    requiredFields: ['name'],
    mappingFields: [
...
const currentOption = computed(() => importOptions.value.find(o => o.type === selectedType.value))
const mappingFields = computed(() => currentOption.value?.mappingFields || [])
const requiredFields = computed(() => currentOption.value?.requiredFields || [])

const isMappingValid = computed(() => {
  if (!currentOption.value) return false
  return currentOption.value.requiredFields.every(field => columnMapping.value[field] && columnMapping.value[field] !== '')
})
...
const processFile = async () => {
  ...
  try {
    ...
    if (selectedType.value === 'customers') {
      fileColumns.value = ['الاسم', 'الهاتف', 'البريد الإلكتروني', 'العنوان', 'الرصيد الافتتاحي']
    } else if (selectedType.value === 'products') {
      fileColumns.value = ['اسم المنتج', 'الباركود', 'معرف الفئة', 'سعر البيع', 'الكمية الحالية']
    } else if (selectedType.value === 'categories') {
      fileColumns.value = ['اسم الفئة', 'الوصف']
    }
    columnMapping.value = {}
    mappingFields.value.forEach(field => { columnMapping.value[field.key] = '' })
...
const importData = async () => {
  ...
  try {
    ...
    importResults.value = {
      success: 85, errors: 5, warnings: 3,
      errorDetails: [
...
const cancelImport = () => resetImport()
