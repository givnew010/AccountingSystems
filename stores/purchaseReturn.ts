...
export const usePurchaseReturnStore = defineStore('purchaseReturn', () => {
  ...
  const invoices = ref<any[]>([
    {
      ...
      originalInvoiceNo: 'PUR-001',
      returnReason: 'defective',
      notes: '',
      items: [{ productId: 1, unit: 'piece', quantity: 2, price: 8.0, total: 16.0 }],
      total: 16.0,
      status: 'draft'
    }
  ])
  ...
  const currentData = ref<any>({ ...getDefaultValues(PurchaseReturnStructure), items: [] })
  ...
  function selectItem(id: number | null) {
    const invoice = invoices.value.find(inv => inv.id === id) ?? ({ ...getDefaultValues(PurchaseReturnStructure), items: [] } as any)
  ...
  function newInvoice() {
    ...
    currentData.value = { ...getDefaultValues(PurchaseReturnStructure), number: nextNumber, items: [] }
  }
